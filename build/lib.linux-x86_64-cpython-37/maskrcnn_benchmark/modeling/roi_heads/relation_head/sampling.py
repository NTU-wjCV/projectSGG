...
class RelationSampling(object):
    ...
    def motif_rel_fg_bg_sampling(self, device, tgt_rel_matrix, ious, is_match, rel_possibility, tgt_rel_importance_matrix, tgt_sub_label_matrix, tgt_obj_label_matrix):
        """
        ...
        for i in range(num_tgt_rels):
            ...
            fg_rel_importance = torch.tensor([rel_importance_value]*prp_tail_idxs.shape[0], dtype=torch.float32, device=device).view(-1,1)
            fg_rel_importance_i = cat((prp_head_idxs.view(-1,1), prp_tail_idxs.view(-1,1), fg_rel_importance), dim=-1).to(torch.float32)
            fg_sub_label = torch.tensor([sub_label_value] * prp_tail_idxs.shape[0], dtype=torch.float32,
                                             device=device).view(-1, 1)
            fg_sub_label_i = cat((prp_head_idxs.view(-1, 1), prp_tail_idxs.view(-1, 1), fg_sub_label),
                                      dim=-1).to(torch.float32)
            fg_obj_label = torch.tensor([obj_label_value] * prp_tail_idxs.shape[0], dtype=torch.float32,
                                        device=device).view(-1, 1)
            fg_obj_label_i = cat((prp_head_idxs.view(-1, 1), prp_tail_idxs.view(-1, 1), fg_obj_label),
                                 dim=-1).to(torch.float32)
        ...
        if len(fg_rel_triplets) == 0:
            ...
            fg_rel_importance_triplets = cat(fg_rel_importance_triplets, dim=0).to(torch.float32)
            fg_sub_label_triplets = cat(fg_sub_label_triplets, dim=0).to(torch.float32)
            fg_obj_label_triplets = cat(fg_obj_label_triplets, dim=0).to(torch.float32)
        ...
        bg_rel_importance_triplets = cat((bg_rel_inds, bg_rel_importance.view(-1,1)), dim=-1).to(torch.float32)
        bg_sub_label = torch.zeros(bg_rel_inds.shape[0], dtype=torch.float32, device=device)
        bg_sub_label_triplets = cat((bg_rel_inds, bg_sub_label.view(-1, 1)), dim=-1).to(torch.float32)
        bg_obj_label = torch.zeros(bg_rel_inds.shape[0], dtype=torch.float32, device=device)
        bg_obj_label_triplets = cat((bg_rel_inds, bg_obj_label.view(-1, 1)), dim=-1).to(torch.float32)
        ...
        return cat((fg_rel_triplets, bg_rel_triplets), dim=0), binary_rel, cat((fg_rel_importance_triplets, bg_rel_importance_triplets), dim=0), cat((fg_sub_label_triplets, bg_sub_label_triplets), dim=0), cat((fg_obj_label_triplets, bg_obj_label_triplets), dim=0)
