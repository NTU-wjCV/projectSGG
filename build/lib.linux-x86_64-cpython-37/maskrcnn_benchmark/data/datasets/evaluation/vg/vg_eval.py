...
from maskrcnn_benchmark.data.datasets.evaluation.vg.sgg_eval import SGRecall1, SGRecall, SGImpoRecall, SGImportanceRecall, SGImportanceRatioRecall, SGNoGraphConstraintRecall, SGUnImportanceRecall, SGZeroShotRecall, SGNGZeroShotRecall, SGPairAccuracy, SGMeanRecall, SGNGMeanRecall, SGAccumulateRecall
...
def do_vg_evaluation(
    ...
    # print('#' * 150)
    # print("rpredictions的值", predictions)
    # #print("rel_labels的形状", rel_labels.shape)  # shape属性显示张量形状
    # print('#' * 1500)
    ...
    print("Content of 'groundtruths':", groundtruths)
    ...
    if "relations" in iou_types:
        ...
        eval_recall1 = SGRecall1(result_dict)
        eval_recall1.register_container(mode)
        evaluator['eval_recall1'] = eval_recall1
        ...
        # Importance Relationship Recall@K
        eval_recall_impo = SGImpoRecall(result_dict)
        eval_recall_impo.register_container(mode)
        evaluator['eval_recall_impo'] = eval_recall_impo

        # # Importance Relationship Recall@K
        # eval_recall_importance = SGImportanceRecall(result_dict)
        # eval_recall_importance.register_container(mode)
        # evaluator['eval_recall_importance'] = eval_recall_importance

        eval_recall_ratio = SGImportanceRatioRecall(result_dict)
        eval_recall_ratio.register_container(mode)
        evaluator['eval_recall_ratio'] = eval_recall_ratio

        # # unImportance Relationship Recall@K
        # eval_recall_unimpo = SGUnImportanceRecall(result_dict)
        # eval_recall_unimpo.register_container(mode)
        # evaluator['eval_recall_unimpo'] = eval_recall_unimpo
        ...
        # eval_mean_recall_impo = SGImportanceMeanRecall(result_dict, num_rel_category, dataset.ind_to_predicates, print_detail=True)
        # eval_mean_recall_impo.register_container(mode)
        # evaluator['eval_mean_recall_impo'] = eval_mean_recall_impo
        ...
        # eval_mean_recall_impo.calculate_mean_recall(mode)
        eval_ng_mean_recall.calculate_mean_recall(mode)
        
        # print result
        result_str += eval_recall1.generate_print_string(mode)
        result_str += eval_recall.generate_print_string(mode)
        result_str += eval_recall_impo.generate_print_string(mode)
        # result_str += eval_recall_importance.generate_print_string(mode)
        result_str += eval_recall_ratio.generate_print_string(mode)
        # result_str += eval_recall_unimpo.generate_print_string(mode)
        ...
        # result_str += eval_mean_recall_impo.generate_print_string(mode)
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator):
    ...
    local_container = {}
    local_container['gt_rels'] = groundtruth.get_field('relation_tuple').detach().cpu().numpy()
    local_container['relation_importance'] = groundtruth.get_field('relation_importance').detach().cpu().numpy()
    ...
    evaluator['eval_recall1'].calculate_recall(global_container, local_container, mode)
    evaluator['eval_recall_impo'].calculate_recall(global_container, local_container, mode)
    # evaluator['eval_recall_importance'].calculate_recall(global_container, local_container, mode)
    evaluator['eval_recall_ratio'].calculate_recall(global_container, local_container, mode)
    # evaluator['eval_recall_unimpo'].calculate_recall(global_container, local_container, mode)
    # No Graph Constraint
    ...
    # Importance Mean Recall
    # evaluator['eval_mean_recall_impo'].collect_mean_recall_items(global_container, local_container, mode)
