...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
        ...
        if self.custom_eval:
            ...
            self.split_mask, self.gt_boxes, self.gt_classes, self.gt_attributes, self.relationships, self.sub_label, self.obj_label = load_graphs(
    ...
    def get_groundtruth(self, index, evaluation=False, flip_img=False):
        ...
        relation = self.relationships[index].copy()  # (num_rel, 4)
        sub_label = self.sub_label[index].copy()
        obj_label = self.obj_label[index].copy()

        if self.filter_duplicate_rels:
            # Filter out dupes!
            assert self.split == 'train'
            all_rel_sets = defaultdict(list)
            for (o0, o1, r, i) in relation:
                all_rel_sets[(o0, o1)].append((r, i))
            relation = [(k[0], k[1], np.random.choice([x[0] for x in v]), np.random.choice([x[1] for x in v])) for k, v
                        in all_rel_sets.items()]
            relation = np.array(relation, dtype=np.float32)

        if self.filter_duplicate_rels:
            # Filter out dupes!
            assert self.split == 'train'
            all_rel_sets1 = defaultdict(list)
            for (o0, o1, r) in sub_label:
                all_rel_sets1[(o0, o1)].append(r)
            sub_label = [(k[0], k[1], np.random.choice(v)) for k, v in all_rel_sets1.items()]
            sub_label = np.array(sub_label, dtype=np.float32)

        if self.filter_duplicate_rels:
            # Filter out dupes!
            assert self.split == 'train'
            all_rel_sets2 = defaultdict(list)
            for (o0, o1, r) in obj_label:
                all_rel_sets2[(o0, o1)].append(r)
            obj_label = [(k[0], k[1], np.random.choice(v)) for k, v in all_rel_sets2.items()]
            obj_label = np.array(obj_label, dtype=np.float32)

        num_box = len(target)
        relation_map = torch.zeros((num_box, num_box), dtype=torch.int64)
        sub_label_map = torch.zeros((num_box, num_box), dtype=torch.int64)
        obj_label_map = torch.zeros((num_box, num_box), dtype=torch.int64)
        relation_importance_map = torch.zeros((num_box, num_box), dtype=torch.float32)  # 新增
        for i in range(relation.shape[0]):
            if relation_map[int(relation[i, 0]), int(relation[i, 1])] > 0:
                if (random.random() > 0.5):
                    relation_map[int(relation[i, 0]), int(relation[i, 1])] = int(relation[i, 2])
                    sub_label_map[int(sub_label[i, 0]), int(sub_label[i, 1])] = int(sub_label[i, 2])
                    obj_label_map[int(obj_label[i, 0]), int(obj_label[i, 1])] = int(obj_label[i, 2])
                    relation_importance_map[int(relation[i, 0]), int(relation[i, 1])] = float(relation[i, 3])  # 新增
            else:
                relation_map[int(relation[i, 0]), int(relation[i, 1])] = int(relation[i, 2])
                sub_label_map[int(sub_label[i, 0]), int(sub_label[i, 1])] = int(sub_label[i, 2])
                obj_label_map[int(obj_label[i, 0]), int(obj_label[i, 1])] = int(obj_label[i, 2])
        ...
        target.add_field("sub_label", sub_label_map, is_triplet=True)
        target.add_field("obj_label", obj_label_map, is_triplet=True)
        target.add_field("relation_importance", relation_importance_map, is_triplet=True)  # 新增

        if evaluation:
            target = target.clip_to_image(remove_empty=False)
            target.add_field("relation_tuple", torch.FloatTensor(relation))  # for evaluation
            return target
        else:
...
def get_VG_statistics(img_dir, roidb_file, dict_file, image_file, must_overlap=True):
    ...
    for ex_ind in tqdm(range(len(train_data))):
        ...
        # print("Content of 'gt_relations':", gt_relations)
        # For the foreground, we'll just look at everything
        o1o2 = gt_classes[gt_relations[:, :2].astype(int)]
        for (o1, o2), gtr in zip(o1o2, gt_relations[:, 2]):
            fg_matrix[o1, o2, int(gtr)] += 1
        # print("Content of 'o1o2':", o1o2)
        # print("Shape of 'o1o2':", o1o2.shape)
        # print("Content of 'fg_matrix':", fg_matrix)
        # print("Shape of 'fg_matrix':", fg_matrix.shape)
...
def load_graphs(roidb_file, split, num_im, num_val_im, filter_empty_rels, filter_non_overlap):
    ...
    Return:
        ...
        relationships: List where each element is a [num_r, 4] array of
                    (box_ind_1, box_ind_2, predicate, importance) relationships
        sub_label: List where each element is a [num_r, 3] array of
                    (box_ind_1, box_ind_2, class_1)
        obj_label: List where each element is a [num_r, 3] array of
                    (box_ind_1, box_ind_2, class_2)
    """
    ...
    relationship_importance = roi_h5['relationship_importance'][:]
    # 确保 relationship_importance 和 _relation_predicates 的长度相同
    assert len(relationship_importance) == len(_relation_predicates)
    ...
    for i in range(len(image_index)):
        ...
        if i_rel_start >= 0:
            ...
            rel_importance = relationship_importance[i_rel_start: i_rel_end + 1]  # 获取对应的 relationship_importance
            assert np.all(obj_idx >= 0)
            assert np.all(obj_idx < boxes_i.shape[0])
            rels = np.column_stack(
                (obj_idx, predicates, rel_importance))  # (num_rel, 4), representing sub, obj, pred and importance

            # 获取物体类别
            obj_classes = all_labels[_relations[i_rel_start: i_rel_end + 1]]
            sub_labels = np.column_stack(
                (obj_idx, obj_classes[:, 0]))  # (num_rel, 3), representing sub, obj, class_1
            obj_labels = np.column_stack(
                (obj_idx, obj_classes[:, 1]))  # (num_rel, 3), representing sub, obj, class_2
        else:
            assert not filter_empty_rels
            rels = np.zeros((0, 4), dtype=np.int32)
            sub_labels = np.zeros((0, 3), dtype=np.int32)
            obj_labels = np.zeros((0, 3), dtype=np.int32)
        ...
        if filter_non_overlap:
            ...
            inc = np.where(rel_overs > 0.0)[0]  # find the overlapping pairs

            if inc.size > 0:
                rels = rels[inc]
                sub_labels = sub_labels[inc]
                obj_labels = obj_labels[inc]
            else:
        ...
        sub_label.append(sub_labels)
        obj_label.append(obj_labels)
    return split_mask, boxes, gt_classes, gt_attributes, relationships, sub_label, obj_label
