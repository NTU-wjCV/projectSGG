...
class SGRecall1(SceneGraphEvaluation):
    def __init__(self, result_dict):
        super(SGRecall1, self).__init__(result_dict)

    def register_container(self, mode):
        self.result_dict[mode + '_recall1'] = {1: [], 5: [], 10: []}

    def generate_print_string(self, mode):
        result_str = 'SGG eval: '
        for k, v in self.result_dict[mode + '_recall1'].items():
            result_str += '    R @ %d: %.4f; ' % (k, np.mean(v))
        result_str += ' for mode=%s, type=Recall1(Main).' % mode
        result_str += '\n'
        return result_str
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        local_container['pred1_to_gt'] = pred_to_gt

        for k in self.result_dict[mode + '_recall1']:
            # the following code are copied from Neural-MOTIFS
            match = reduce(np.union1d, pred_to_gt[:k])
            rec_i = float(len(match)) / float(gt_rels.shape[0])
            self.result_dict[mode + '_recall1'][k].append(rec_i)
...
class SGRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        return local_container
class SGImpoRecall(SceneGraphEvaluation):
    def __init__(self, result_dict):
        super(SGImpoRecall, self).__init__(result_dict)

    def register_container(self, mode):
        self.result_dict[mode + '_recall_im'] = {1: [], 5: [], 10: []}

    def generate_print_string(self, mode):
        result_str = 'SGG eval: '
        for k, v in self.result_dict[mode + '_recall_im'].items():
            result_str += ' im-R @ %d: %.4f; ' % (k, np.mean(v))
        result_str += ' for mode=%s, type=Impo Relationship Recall(Main).' % mode
        result_str += '\n'
        return result_str
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        iou_thres = global_container['iou_thres']
        # Get the importance values from the fourth column
        importance_values = gt_rels[:, 3]
        # Create a boolean index array for important relations
        important_inds = importance_values > 0.5
        unimportant_inds = importance_values <= 0.5
        # Filter out the unimportant ground truth relations
        important_gt_rels = gt_rels[important_inds]  # .reshape(1, -1)
        pred_rels = np.column_stack((pred_rel_inds, 1 + rel_scores[:, 1:].argmax(1)))
        pred_scores = rel_scores[:, 1:].max(1)
        ...
        gt_triplets, gt_triplet_boxes, _ = _triplet(important_gt_rels, gt_classes, gt_boxes)
        local_container['gt_triplets'] = gt_triplets
        local_container['gt_triplet_boxes'] = gt_triplet_boxes
        ...
        local_container['im_pred_to_gt'] = pred_to_gt

        for k in self.result_dict[mode + '_recall_im']:
            # the following code are copied from Neural-MOTIFS
            match = reduce(np.union1d, pred_to_gt[:k])
            if important_gt_rels.shape[0] == 0:
                rec_i = 0
            else:
                rec_i = float(len(match)) / float(important_gt_rels.shape[0])
            self.result_dict[mode + '_recall_im'][k].append(rec_i)

        return local_container

class SGImportanceRecall(SceneGraphEvaluation):
    def __init__(self, result_dict):
        super(SGImportanceRecall, self).__init__(result_dict)

    def register_container(self, mode):
        self.result_dict[mode + '_recall_impo'] = {1: [], 5: [], 10: []}

    def generate_print_string(self, mode):
        result_str = 'SGG eval: '
        for k, v in self.result_dict[mode + '_recall_impo'].items():
            result_str += ' impo-R @ %d: %.4f; ' % (k, np.mean(v))
        result_str += ' for mode=%s, type=Importance Relationship Recall(Main).' % mode
        result_str += '\n'
        return result_str
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        unimportant_inds = importance_values <= 0.5
        # Filter out the unimportant ground truth relations
        important_gt_rels = gt_rels[important_inds]
        ...
        local_container['impo_pred_to_gt'] = pred_to_gt

        for k in self.result_dict[mode + '_recall_impo']:
            # the following code are copied from Neural-MOTIFS
            match = reduce(np.union1d, pred_to_gt[:k])
            rec_i = float(len(match)) / float(gt_rels.shape[0])
            self.result_dict[mode + '_recall_impo'][k].append(rec_i)

        return local_container

class SGImportanceRatioRecall(SceneGraphEvaluation):
    def __init__(self, result_dict):
        super(SGImportanceRatioRecall, self).__init__(result_dict)

    def register_container(self, mode):
        self.result_dict[mode + '_recall_ratio'] = {1: [], 5: [], 10: []}

    def generate_print_string(self, mode):
        result_str = 'SGG eval: '
        for k, v in self.result_dict[mode + '_recall_ratio'].items():
            result_str += '    R ratio @ %d: %.4f; ' % (k, np.mean(v))
        result_str += ' for mode=%s, type=Importance Relationship Recall Ratio.' % mode
        result_str += '\n'
        return result_str

    def calculate_recall(self, global_container, local_container, mode):
        for k in self.result_dict[mode + '_recall_ratio']:
            recall_total = np.mean(self.result_dict[mode + '_recall1'][k])
            recall_importance = np.mean(self.result_dict[mode + '_recall_im'][k])
            if recall_total == 0:
                recall_ratio = 0.0  # »òÕßÑ¡ÔñÆäËûÊÊµ±µÄÄ¬ÈÏÖµ
            else:
                recall_ratio = recall_importance / recall_total

            self.result_dict[mode + '_recall_ratio'][k].append(recall_ratio)
class SGUnImportanceRecall(SceneGraphEvaluation):
    def __init__(self, result_dict):
        super(SGUnImportanceRecall, self).__init__(result_dict)

    def register_container(self, mode):
        self.result_dict[mode + '_recall_unimpo'] = {20: [], 50: [], 100: []}

    def generate_print_string(self, mode):
        result_str = 'SGG eval: '
        for k, v in self.result_dict[mode + '_recall_unimpo'].items():
            result_str += ' unim-R @ %d: %.4f; ' % (k, np.mean(v))
        result_str += ' for mode=%s, type=unImportance Relationship Recall(Main).' % mode
        result_str += '\n'
        return result_str
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        unimportant_inds = importance_values <= 0.5
        # Filter out the unimportant ground truth relations
        important_gt_rels = gt_rels[unimportant_inds]
        ...
        local_container['unimpo_pred_to_gt'] = pred_to_gt

        for k in self.result_dict[mode + '_recall_unimpo']:
            # the following code are copied from Neural-MOTIFS
            match = reduce(np.union1d, pred_to_gt[:k])
            rec_i = float(len(match)) / float(gt_rels.shape[0])
            self.result_dict[mode + '_recall_unimpo'][k].append(rec_i)
...
class SGZeroShotRecall(SceneGraphEvaluation):
    ...
    def prepare_zeroshot(self, global_container, local_container):
        ...
        gt_triplets = np.column_stack((gt_classes[sub_id.astype(int)], gt_classes[ob_id.astype(int)], pred_label))  # num_rel, 3
...
def _triplet(relations, classes, boxes, predicate_scores=None, class_scores=None):
    ...
    triplets = np.column_stack((classes[sub_id.astype(int)], pred_label, classes[ob_id.astype(int)]))
    triplet_boxes = np.column_stack((boxes[sub_id.astype(int)], boxes[ob_id.astype(int)]))

    triplet_scores = None
    if predicate_scores is not None and class_scores is not None:
        triplet_scores = np.column_stack((
            class_scores[sub_id.astype(int)], predicate_scores, class_scores[ob_id.astype(int)],
        ))
