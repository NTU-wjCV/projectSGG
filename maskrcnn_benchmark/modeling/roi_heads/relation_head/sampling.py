...
class RelationSampling(object):
    ...
    def gtbox_relsample(self, proposals, targets):
        ...
        for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
            ...
            tgt_rel_importance = tgt_rel_importance_matrix[tgt_head_idxs, tgt_tail_idxs].contiguous().view(-1)
            ...
            # rel_obj_classes
            obj_classes = torch.zeros((tgt_pair_idxs.shape[0], 2), device=device)
            obj_classes[:, 0] = tgt_sub_label_matrix[tgt_head_idxs, tgt_tail_idxs].contiguous().view(-1)
            obj_classes[:, 1] = tgt_obj_label_matrix[tgt_head_idxs, tgt_tail_idxs].contiguous().view(-1)
            rel_obj_classes.append(obj_classes)
            ...
            if tgt_pair_idxs.shape[0] > num_pos_per_img:
                ...
                tgt_rel_importance = tgt_rel_importance[perm]
                obj_classes = obj_classes[perm]  # 更新 obj_classes 以保持一致性
            ...
            img_rel_importance = torch.cat((tgt_rel_importance, torch.zeros(tgt_bg_idxs.shape[0], device=device)),
                                           dim=0).contiguous().view(-1)
            rel_idx_pairs.append(img_rel_idxs)
            rel_labels.append(img_rel_labels)
            rel_importance.append(img_rel_importance)

        return proposals, rel_labels, rel_idx_pairs, rel_sym_binarys, rel_importance, rel_obj_classes
    ...
    def detect_relsample(self, proposals, targets):
        ...
        for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
            ...
            if self.require_overlap and (not self.use_gt_box):
                ...
                # È·±£ prp_lab µÄÖµÔÚ rel_possibility µÄË÷Òý·¶Î§ÄÚ
            if (prp_lab < 0).any() or (prp_lab >= rel_possibility.size(0)).any():
                valid_mask = (prp_lab < rel_possibility.size(0))
                prp_lab = prp_lab[valid_mask]
                rel_possibility = rel_possibility[valid_mask][:, valid_mask]
            # only select relations between fg proposals
            rel_possibility[prp_lab == 0] = 0
            rel_possibility[:, prp_lab == 0] = 0

            img_rel_triplets, binary_rel, img_rel_importance_triplets, img_sub_label_triplets, img_obj_label_triplets = self.motif_rel_fg_bg_sampling(
                device, tgt_rel_matrix, ious, is_match, rel_possibility, tgt_rel_importance_matrix, tgt_sub_label_matrix, tgt_obj_label_matrix
            )
            rel_idx_pairs.append(img_rel_triplets[:, :2])  # (num_rel, 2),  (sub_idx, obj_idx)
            rel_labels.append(img_rel_triplets[:, 2])  # (num_rel, )
            rel_importance.append(img_rel_importance_triplets[:, 2])
            obj_classes = torch.zeros((img_rel_triplets.shape[0], 2), device=device)
            obj_classes[:, 0] = img_sub_label_triplets[:, 2]
            obj_classes[:, 1] = img_obj_label_triplets[:, 2]
            rel_sym_binarys.append(binary_rel)
            rel_obj_classes.append(obj_classes)
    ...
    def motif_rel_fg_bg_sampling(self, device, tgt_rel_matrix, ious, is_match, rel_possibility,
                                 tgt_rel_importance_matrix, tgt_sub_label_matrix, tgt_obj_label_matrix):
        tgt_pair_idxs = torch.nonzero(tgt_rel_matrix > 0)
        assert tgt_pair_idxs.shape[1] == 2
        tgt_head_idxs = tgt_pair_idxs[:, 0].contiguous().view(-1)
        tgt_tail_idxs = tgt_pair_idxs[:, 1].contiguous().view(-1)
        tgt_rel_labs = tgt_rel_matrix[tgt_head_idxs, tgt_tail_idxs].contiguous().view(-1)
        tgt_rel_importance = tgt_rel_importance_matrix[tgt_head_idxs, tgt_tail_idxs].contiguous().view(-1)
        tgt_sub_label = tgt_sub_label_matrix[tgt_head_idxs, tgt_tail_idxs].contiguous().view(-1)
        tgt_obj_label = tgt_obj_label_matrix[tgt_head_idxs, tgt_tail_idxs].contiguous().view(-1)
        ...
        fg_rel_triplets = []
        fg_rel_importance_triplets = []
        fg_sub_label_triplets = []
        fg_obj_label_triplets = []
        ...
        for i in range(num_tgt_rels):
            ...
            rel_importance_value = float(tgt_rel_importance[i])
            sub_label_value = float(tgt_sub_label[i])
            obj_label_value = float(tgt_obj_label[i])
            ...
            if (prp_head_idxs >= rel_possibility.size(0)).any() or (prp_tail_idxs >= rel_possibility.size(1)).any():
                # print(f"prp_head_idxs ³¬³ö·¶Î§: {prp_head_idxs}")
                # print(f"prp_tail_idxs ³¬³ö·¶Î§: {prp_tail_idxs}")
                continue
            ...
            fg_rel_importance = torch.tensor([rel_importance_value] * prp_tail_idxs.shape[0], dtype=torch.float32,
                                             device=device).view(-1, 1)
            fg_rel_importance_i = torch.cat((prp_head_idxs.view(-1, 1), prp_tail_idxs.view(-1, 1), fg_rel_importance),
                                            dim=-1).to(torch.float32)

            fg_sub_label = torch.tensor([sub_label_value] * prp_tail_idxs.shape[0], dtype=torch.float32,
                                        device=device).view(-1, 1)
            fg_sub_label_i = torch.cat((prp_head_idxs.view(-1, 1), prp_tail_idxs.view(-1, 1), fg_sub_label), dim=-1).to(
                torch.float32)

            fg_obj_label = torch.tensor([obj_label_value] * prp_tail_idxs.shape[0], dtype=torch.float32,
                                        device=device).view(-1, 1)
            fg_obj_label_i = torch.cat((prp_head_idxs.view(-1, 1), prp_tail_idxs.view(-1, 1), fg_obj_label), dim=-1).to(
                torch.float32)
            ...
            if fg_rel_i.shape[0] > self.num_sample_per_gt_rel:
                ...
                fg_rel_importance_i = fg_rel_importance_i[perm]
                fg_sub_label_i = fg_sub_label_i[perm]
                fg_obj_label_i = fg_obj_label_i[perm]

            if fg_rel_i.shape[0] > 0:
                fg_rel_triplets.append(fg_rel_i)
                fg_rel_importance_triplets.append(fg_rel_importance_i)
                fg_sub_label_triplets.append(fg_sub_label_i)
                fg_obj_label_triplets.append(fg_obj_label_i)

        if len(fg_rel_triplets) == 0:
            fg_rel_triplets = torch.zeros((0, 3), dtype=torch.int64, device=device)
            fg_sub_label_triplets = torch.zeros((0, 3), dtype=torch.float32, device=device)
            fg_obj_label_triplets = torch.zeros((0, 3), dtype=torch.float32, device=device)
            fg_rel_importance_triplets = torch.zeros((0, 3), dtype=torch.float32, device=device)
        else:
            fg_rel_triplets = torch.cat(fg_rel_triplets, dim=0).to(torch.int64)
            fg_rel_importance_triplets = torch.cat(fg_rel_importance_triplets, dim=0).to(torch.float32)
            fg_sub_label_triplets = torch.cat(fg_sub_label_triplets, dim=0).to(torch.float32)
            fg_obj_label_triplets = torch.cat(fg_obj_label_triplets, dim=0).to(torch.float32)
            if fg_rel_triplets.shape[0] > self.num_pos_per_img:
                ...
                fg_rel_importance_triplets = fg_rel_importance_triplets[perm]
                fg_sub_label_triplets = fg_sub_label_triplets[perm]
                fg_obj_label_triplets = fg_obj_label_triplets[perm]
        ...
        bg_rel_importance = torch.zeros(bg_rel_inds.shape[0], dtype=torch.float32, device=device)
        bg_rel_importance_triplets = torch.cat((bg_rel_inds, bg_rel_importance.view(-1, 1)), dim=-1).to(torch.float32)
        bg_sub_label = torch.zeros(bg_rel_inds.shape[0], dtype=torch.float32, device=device)
        bg_sub_label_triplets = torch.cat((bg_rel_inds, bg_sub_label.view(-1, 1)), dim=-1).to(torch.float32)
        bg_obj_label = torch.zeros(bg_rel_inds.shape[0], dtype=torch.float32, device=device)
        bg_obj_label_triplets = torch.cat((bg_rel_inds, bg_obj_label.view(-1, 1)), dim=-1).to(torch.float32)
        ...
        if bg_rel_triplets.shape[0] > 0:
            ...
            bg_rel_importance_triplets = bg_rel_importance_triplets[perm]
            bg_sub_label_triplets = bg_sub_label_triplets[perm]
            bg_obj_label_triplets = bg_obj_label_triplets[perm]
        else:
            bg_rel_triplets = torch.zeros((0, 3), dtype=torch.int64, device=device)
            bg_rel_importance_triplets = torch.zeros((0, 3), dtype=torch.float32, device=device)
            bg_sub_label_triplets = torch.zeros((0, 3), dtype=torch.float32, device=device)
            bg_obj_label_triplets = torch.zeros((0, 3), dtype=torch.float32, device=device)
        ...
        if fg_rel_triplets.shape[0] == 0 and bg_rel_triplets.shape[0] == 0:
            ...
            bg_rel_importance_triplets = torch.zeros((1, 3), dtype=torch.float32, device=device)
            bg_sub_label_triplets = torch.zeros((1, 3), dtype=torch.float32, device=device)
            bg_obj_label_triplets = torch.zeros((1, 3), dtype=torch.float32, device=device)

        return torch.cat((fg_rel_triplets, bg_rel_triplets), dim=0), binary_rel, torch.cat(
            (fg_rel_importance_triplets, bg_rel_importance_triplets), dim=0), torch.cat(
            (fg_sub_label_triplets, bg_sub_label_triplets), dim=0), torch.cat(
            (fg_obj_label_triplets, bg_obj_label_triplets), dim=0)
