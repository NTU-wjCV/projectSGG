...
@registry.ROI_RELATION_PREDICTOR.register("myNetwork_im")
class myNetwork_im(nn.Module):
    def __init__(self, config, in_channels):
        super(myNetwork_im, self).__init__()
        # 获取配置文件中的参数
        ...
        self.norm_pair1 = nn.Sequential(*[
            ...
            nn.BatchNorm1d(self.mlp_dim)   # self.mlp_dim // 2
        ])
        ...
        # 添加注意力层
        self.poolformer_attention = SequencePoolFormer(embed_dim=self.mlp_dim, depth=3)

        # 初始化重要性分数预测模块
        self.importance_predictor = ImportancePredictor(self.mlp_dim//2)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, rel_importance,
        ...
        for pair_idx, sub_rep, obj_rep, entity_pred, entity_embed, proposal, roi_feat in zip(rel_pair_idxs, sub_reps,
                                                                                             obj_reps, entity_preds,
                                                                                             entity_embeds, proposals,
                                                                                             roi_features):
        ...
        fusion_uo = self.fusion_uo(obj_list)
        #####
        ...
        # 预测重要性分数
        pred_importance_scores = self.importance_predictor(rel_rep)
        #print('pred_importance_scores',pred_importance_scores)
        ...
        if self.training:
            # mse损失
            importance_loss = F.mse_loss(pred_importance_scores, rel_importance.unsqueeze(1))
            add_losses.update({"importance_loss": importance_loss})
        ...
        return entity_dists, rel_dists, add_losses, add_data, pred_importance_scores
...
@registry.ROI_RELATION_PREDICTOR.register("PrototypeEmbeddingNetwork_im")
class PrototypeEmbeddingNetwork_im(nn.Module):
    def __init__(self, config, in_channels):
        super(PrototypeEmbeddingNetwork_im, self).__init__()
