...
class WeightedLabelSmoothingLoss(Label_Smoothing_Regression):
...
class RelationLossComputation(object):
    ...
    def __init__(
        ...
        if self.use_label_smoothing:
            self.criterion_loss_rel = WeightedLabelSmoothingLoss()
        else:
            self.criterion_loss_rel = WeightedCrossEntropyLoss()

    def __call__(self, proposals, rel_labels, relation_logits, refine_logits, rel_importance):
        """
        Computes the loss for relation triplet.
        ...
        rel_importance = cat(rel_importance, dim=0)

        loss_relation = self.criterion_loss_rel(relation_logits, rel_labels.long(), rel_importance)
