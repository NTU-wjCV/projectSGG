...
class ROIRelationHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        if self.training:
            ...
            with torch.no_grad():
                if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
                    proposals, rel_labels, rel_pair_idxs, rel_binarys, rel_importance, rel_obj_classes = self.samp_processor.gtbox_relsample(proposals, targets)
                else:
                    proposals, rel_labels, rel_pair_idxs, rel_binarys, rel_importance, rel_obj_classes= self.samp_processor.detect_relsample(proposals, targets)
        else:
            rel_labels, rel_binarys, rel_importance, rel_obj_classes = None, None, None, None
        ...
        refine_logits, relation_logits, add_losses, add_data = self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, rel_importance, rel_obj_classes, logger)

        # for test
        if not self.training:
            # relation_logits = cat(relation_logits, dim=0)
            # pred_importance_scores = pred_importance_scores.squeeze(1)
            # _, sorted_indices = torch.sort(pred_importance_scores, descending=True)
            # relation_logits = relation_logits[sorted_indices]
            # rel_pair_idxs = [rel_pair_idxs[0][sorted_indices]]
        ...
        loss_relation, loss_refine = self.loss_evaluator(proposals, rel_labels, relation_logits, refine_logits, rel_importance)
